...
| **`type_text`** | Uses verified application-targeted delivery when a target and semantic selector are supplied; otherwise types into the active app. | `text`, `appName` or `windowId`, `selector`, `executionMode` |
| **`press_key`** | Posts supported keys to a semantic editable control in `background_required` mode, or uses the active app in `foreground_allowed` mode. | `key`, `appName` or `windowId`, `selector`, `executionMode`, `modifiers` |
...
| **`launch_app`** | Launches in the background by default. Foreground activation requires `activate: true`. | `appName`, `activate` |
...
3. **Automation**: Requested only when you explicitly call `run_applescript`. Core input, background tools, and active-app key presses use the native helper instead of System Events.
...
Coordinate-based mutating actions return `status: "delivered_unverified"`. Semantic background tools return errors when the target or action cannot be verified. Continue to verify consequential post-state with semantic inspection, OCR, or a screenshot.
